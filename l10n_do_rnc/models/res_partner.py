...
class Partner(models.Model):
        ...
        for val in vals_list:
            ...
            if val.get('country_id', False) == self.env.ref('base.do').id and rnc and rnc.isdigit():
                ...
                if contact_exist:
                    ...
                    contact_exist.name, _('ID') if len(rnc) == 11 else _('RNC'), rnc))
    ...
    def get_name_from_dgii(self, vat):
        if (len(vat) not in [9, 11]):
            raise UserError(
                _('Please check the RNC/Cedula, it does not have the appropriate number of digits, only enter numbers (without hyphens), 9 digits for RNC and 11 digits for Cedula.'))
